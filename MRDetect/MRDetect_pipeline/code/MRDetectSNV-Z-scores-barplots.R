...
packages <- c("data.table", "rebus", "dplyr", "plyr", "ggplot2", "stringr", "stringi")
...
for (i in 1:(length(args)-1)) {
  ...
  case_zscore[[i]] <- list()
  filesin[[i]] <- list.files(path = paste0(args[i+1], "/"), pattern = "_RESULT" %R% DOT %R% "csv" %R% END)
  ...
  for (j in 1:length(filesin[[i]])){
    tsv_list[[i]][[j]] <- read.csv(paste0(paste0(args[i+1], "/"), filesin[[i]][j]), header = TRUE, nrows = 1)
    sites_detected[[i]][[j]] <- tsv_list[[i]][[j]]$sites.detected
