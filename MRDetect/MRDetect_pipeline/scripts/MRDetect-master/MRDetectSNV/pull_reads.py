...
class FilestreamVCF():
	...
	def sites(self):
		for line in self.line_iterator():
			try:
				site = {"CHROM" : line[0], "POS" : int(line[1]), "REF" : line[3].upper(), "ALT" : line[4].upper()}
				yield site
...
def site_reads(bam, site, filter_dups = False, mapped = False, match_alt = False):
	...
	"""
	for read in bam.fetch(site["CHROM"], start = site["POS"]-50, stop = site["POS"]+50):
...
def write_read_features(read, site, tsv):
	"""
	Writes a read with SVM features to a TSV.
	"""
	orient = read.is_read1*1
	template_pos = read.is_reverse*1
	RID = read.query_name
	...
	line = [str(l) for l in [site["CHROM"], site["POS"]-1, site["REF"],alt,RID,MRBQ,BQ,MQ,PIR, frag, template_pos, orient]]
	line = "\t".join(line) + "\n"
	tsv.write(line)
...
def compare_TSVs(inputs):
	...
	columns = ["chrom", "pos", "ref", "alt", "rid", "mrbq", "vbq", "mq", "pir", "frag", "t_pos", "orient"]
...
def main():
	...
	Main
	"""
	parser = argparse.ArgumentParser(description='MRDetect')
