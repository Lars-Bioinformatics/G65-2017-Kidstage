...
rule gc_wiggle:
    ...
    shell:
        """
        python2 {WORK}udocker/udocker run \
        --user=pi \
        --volume=/work:/work \
        sequenza \
        bash -c 'sequenza-utils gc_wiggle --fasta {input.ref} -w {window} -o {output.wig}'
        """
...
rule bam2seqz:
    ...
    shell:
        ...
        --volume=/work:/work \
        sequenza \
        sequenza-utils bam2seqz -n {input.normal} -t {input.tumor} -q 20 -N 20 -f "illumina" --fasta {ref} -gc {wig} -o {output}
        """
...
rule sequenza_analysis:
    ...
    params:
        normal="{normal}",
        tumor="{tumor}",
        outdir=OUTPUT+"Sequenza_{tumor}_vs_{normal}_bin{window}"
    threads: 2
    shell:
        """
        python2 {WORK}udocker/udocker run \
        ...
        --volume=/work:/work \
        sequenza \
        Rscript {WORK}scripts/sequenza_analysis.R {input.seqz_bin} {params.normal} {params.tumor} {params.outdir}
        """
...
###############################################################################
#### Sequenza-docker complete pipeline script - untested
###############################################################################
#rule sequenza_pipeline:
#    input:
#        normal=INPUT_BAM+"{normal}.connor.recalibrated.bam",
#        tumor=INPUT_BAM+"{tumor}.connor.recalibrated.bam",
#        ref = ref,
...
#        --volume=/work:/work \
#        sequenza \
#        bash -c 'sequenza-pipeline \
#            --sample-id Sequenza_{wildcards.tumor}_vs_{wildcards.normal} \
#            --normal-bam {input.normal} \
#            --tumor-bam {input.tumor} \
#            --reference-gz {input.ref} \
#            --gc_wig {input.wig}'
#        cp /tmp/workdir/* {OUTPUT}
#        """