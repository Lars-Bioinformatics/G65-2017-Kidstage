...
'''
rule Mutect2_matched_calling_forced:
    input:
        ...
        force_intervals=lambda wildcards: expand(main_output+"union_vcf/{patient}"+force_intervals_suffix, patient=wildcards.normal.replace("G65-B","G65-T"))
    output:
        vcf="{output}vcf_files/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz",
        idx="{output}vcf_files/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz.tbi",
        realigned_bam="{output}realigned_bam/{tumor}_vs_{normal}_somatic_mutect2-forced.bam",
        vcf_stats="{output}vcf_files/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz.stats",
        f1r2="{output}f1r2/{tumor}_vs_{normal}_f1r2.tar.gz"
...
rule FilterMutectCalls_forced:
    input:
        vcf="{output}vcf_files/{sample}_somatic_mutect2-forced.vcf.gz",
        idx="{output}vcf_files/{sample}_somatic_mutect2-forced.vcf.gz.tbi",
        bam="{output}realigned_bam/{sample}_somatic_mutect2-forced.bam",
        stats="{output}vcf_files/{sample}_somatic_mutect2-forced.vcf.gz.stats",
    ...
    output:
        vcf="{output}vcf_filterFlag/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz",
        tbi="{output}vcf_filterFlag/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz.tbi",
    shell:
        """
...
rule vcf_norm_decompose_bcftools:
    input:
        vcf="{output}vcf_filterFlag/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz"
    output:
        norm_vcf="{output}vcf_filterFlag_norm/{sample}_somatic_mutect2-forced_filterFlag_norm.vcf.gz"
    shell:
        """
...
rule intersect_forcedVCF_mergedVCF:
    input:
        norm_vcf="{output}vcf_filterFlag_norm/{tumor}_vs_{normal}_somatic_mutect2-forced_filterFlag_norm.vcf.gz",
        force_intervals=lambda wildcards: expand(main_output+"union_vcf/{patient}"+force_intervals_suffix, patient=wildcards.normal.replace("G65-B","G65-T"))
    output:
        final_vcf="{output}vcf_final/{tumor}_vs_{normal}__somatic_mutect2-forced_filterFlag_norm_final.vcf.gz"
    shell:
        """
