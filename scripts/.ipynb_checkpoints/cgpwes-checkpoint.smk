...
# General:
# - bam index must end on bam.bai
# - needs a bam stats file ending on bam.bas (use compute_cgp_bamstats.smk)

WORK="/work/sduvarcall/"
INPUT_BAM = WORK+"G65-2017-Kidstage/Connor/bam"
INPUT_BAM_BRASS = WORK+"G65-2017-Kidstage/MarkDuplicates/cgp_bam"
OUTPUT = WORK+"G65-2017-Kidstage/Connor/cgpwes_out"
# INPUT_BAM = WORK+"G65-2017-Kidstage/MarkDuplicates/bam"
# INPUT_BAM_BRASS = WORK+"G65-2017-Kidstage/MarkDuplicates/cgp_bam"
# OUTPUT = WORK+"G65-2017-Kidstage/MarkDuplicates/cgpwes_out"
...
configfile: "/work/sduvarcall/G65-2017-Kidstage/samples-pairwise.yaml"
...
# SAMPLES = ["G65-T12-BDCB_nimblegen-medexome_HYLKFDSXX"]
print(SAMPLES)
...
rule all:
    input:
        ...
        [expand(OUTPUT+"/finished/BRASS_SUCCESS__"+PROTOCOL+"__"+CNV_TOOL+"__{tumor}__vs__{normal}.txt", tumor=config[s]["tumor"], normal=config[s]["normal"]) for s in SAMPLES]
...
checkpoint ascat:
    input:
        normal=INPUT_BAM+"/{normal}.connor.recalibrated.bam",
        tumor=INPUT_BAM+"/{tumor}.connor.recalibrated.bam",
        res1_1=OUTPUT+"/{tumor}/timings/{PROTOCOL}__ascat_{tumor}_vs_{normal}.time.CaVEMan_setup"
    output:
...
rule BRASS_input:
    input:
        normal=INPUT_BAM_BRASS+"/{normal}.bam",
        tumor=INPUT_BAM_BRASS+"/{tumor}.bam"
    output:
...
rule run_brass:
    input:
        res5_1=OUTPUT+"/{tumor}/timings/{PROTOCOL}__{CNV_TOOL}_{tumor}_vs_{normal}.time.BRASS",
    output:
        OUTPUT+"/finished/BRASS_SUCCESS__{PROTOCOL}__{CNV_TOOL}__{tumor}__vs__{normal}.txt"
    shell:
        """
        echo 'Successfully completed BRASS pipeline!' > {output}
        """
